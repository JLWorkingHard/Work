...
### need EDIT!!!!!!!
def parseline(line):
    r = re.compile('\s*(add|mod|rm)\s+["]\s*[a-zA-Z]+[\s|a-z|A-Z]*["](\s+[(]\s*[-]?\d+\s*[,]\s*[-]?\d+\s*[)])*')
    if re.match(r,line.replace('\n','')):
                    ...
                    if cmd.strip() == 'add':
                        if road_name not in s.my_map:
                            ...
                            s.add(road_name, point)
                                ...
                                raise Exception('input road name already exists')
                    elif cmd.strip() == 'mod':
                            if road_name in s.my_map:
                                ...
                                raise Exception('input road name does not exists')
                    else:
                        raise Exception('wrong input')
                else:
                    raise Exception('need at least 2 coordinates')
            elif len(road) == 2: 
                if cmd.strip() == 'rm':
                    if road_name in s.my_map:
                        s.rm(road_name)
        ...
        raise Exception('a valid input is like: add|mod + "street name" + coordinates or rm + "street name"')
...
class Printer(object):
    ...
    def gg(self, verties, edges):
        ...
        for i in range(len(verties)):
            v = '  {0}:  ({1:.2f},{2:.2f})'.format((i+1),verties[i][0],verties[i][1])
            V.append(v)
        V.append('}')
            ...
            if line.replace('\n','') == '':
                ...
                for key in my_map.keys():
                    if len(my_map[key]) != len(set(my_map[key])):
                        raise Exception('Repeatd point @ '+key+', please mod this street again')
